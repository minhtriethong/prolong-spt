function [tracksFinal] = scriptTrackGeneral_func(movieInfo)
...
% 
SearchRadius = 1;
SearchRadius_range = 4;
gapCloseParam.timeWindow = 1; %default = 5; maximum allowed time gap (in frames) between a track segment end and a track segment start that allows linking them.
...
parameters.minSearchRadius = SearchRadius; %default = 6; minimum allowed search radius. The search radius is calculated on the spot in the code given a feature's motion parameters. If it happens to be smaller than this minimum, it will be increased to the minimum.
parameters.maxSearchRadius = SearchRadius + SearchRadius_range; %default = 6; maximum allowed search radius. Again, if a feature's calculated search radius is larger than this maximum, it will be reduced to this maximum.
...
parameters.minSearchRadius = SearchRadius; %default = 6; minimum allowed search radius.
parameters.maxSearchRadius = SearchRadius+SearchRadius_range; %default = 6; maximum allowed search radius.
